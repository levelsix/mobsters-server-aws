...
public interface MonsterService {
	...
	/**
	 * Apply an arbitary number of property changes to an arbitrary number of
	 * monsters all owned by a single user. In the details table, a row
	 * corresponds to the identifier for a specific user monster, a column
	 * corresponds to a specific type of property change operation, and a value
	 * is the argument required to perform the column-specified operation.
	 * 
	 * @param details
	 * @see MonsterForUserOp
	 */
	public abstract void modifyMonstersForUser(String userId,
	...
	public interface ModifyMonstersSpecBuilder {
		...
		ModifyMonstersSpecBuilder setExperienceAbsolute(String monsterId,
				int value);

		ModifyMonstersSpecBuilder setExperienceRelative(String monsterId,
				int delta);
	...
	public class ModifyUserSpec {
		...
		ModifyUsersSpec(Multimap<String, MonsterFunc> specMap) {
			this.specMap = specMap;
		}
