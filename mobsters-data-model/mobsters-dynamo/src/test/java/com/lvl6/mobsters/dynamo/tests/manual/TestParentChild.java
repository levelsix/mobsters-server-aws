...
public class TestParentChild
{
	...
	private static final int NUM_PARENTS = 40;

	private static final int NUM_CHILDREN_PER_PARENT = 40;
	    ...
	    final VariantStrategy<?, ? extends ChildDataAttrs> nextVariant )
	{
		...
		for (int ii = 0; ii < NUM_PARENTS; ii++) {
			parentIds[ii] = variantOne.getNextParent(NUM_CHILDREN_PER_PARENT);
			for (int jj = 0; jj < NUM_CHILDREN_PER_PARENT; jj++) {
				final ChildDataAttrs nextChild = variantOne.addNextChild();
				...
				nextChild.setTradeValue(9.999);
			}
			variantOne.saveChildren();
		}
	...
	public void testSinglePartition()
	{
		...
		System.out.println("Time taken for variant one := "
		    + deltaOne
		    + " seconds to issue "
		    + requestCount
		    + " read all requests for all "
		    + NUM_CHILDREN_PER_PARENT
		    + " children from one of "
		    + READ_INDICES_ORDER.length
		    + " distinct parents from a data set with "
		    + NUM_PARENTS
